...
impl PasswordHasher for Argon2Hasher {
    fn hash(&self, plain: &str) -> String {
        // Salt novo a cada chamada: é o que faz duas contas com a mesma senha
        // terem hashes diferentes, e o que inutiliza uma rainbow table.
        let salt = SaltString::generate(&mut OsRng);

        match self.argon.hash_password(plain.as_bytes(), &salt) {
            Ok(hash) => hash.to_string(),
            // Só falha por parâmetro inválido, que aqui é constante. Devolver
            // string vazia é seguro: ela não é um hash PHC válido, então
            // `verify` recusa qualquer senha contra ela — a conta fica
            // inacessível em vez de aceitar qualquer um.
            Err(_) => String::new(),
        }
    }
    ...
    fn verify(&self, plain: &str, hash: &str) -> bool {
        let Ok(parsed) = PasswordHash::new(hash) else {
            // Hash corrompido ou em formato desconhecido: não há como conferir,
            // e "não sei" tem que significar "não passa".
            return false;
        };
...
mod tests {
    ...
    fn a_mesma_senha_gera_hashes_diferentes() {
        // Se estes coincidissem, o salt não estaria sendo aplicado e o banco
        // inteiro viraria alvo de uma tabela pré-computada só.
        let hasher = Argon2Hasher::new();
