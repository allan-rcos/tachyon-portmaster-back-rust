...
// O ban de `std::sync::Mutex` (.clippy.toml) existe para impedir que estado de
// requisição vire estado global sincronizado — o caminho certo para aquilo é o
// task-local. Aqui é outra coisa: um recurso do **processo**, cuja seção crítica
// é o incremento de um contador e não contém `.await` nenhum. É exatamente o
// caso em que o mutex síncrono é o certo, e o assíncrono seria pior.
#[allow(
