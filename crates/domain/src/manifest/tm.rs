...
//!
//! Toda aritmética aqui é de ponto flutuante, e por isso as comparações passam
//! por [`EPSILON`]. Um contêiner de 1000 kg que recebe 1000 kg exatos pode
//! acabar com 1000.0000000000001 depois de algumas somas — e sem a tolerância a
//! carga que cabe seria recusada por um erro de arredondamento invisível.
...
use crate::container::model::ContainerModel;

use super::model::{ManifestCargo, ManifestCargoModel, ManifestChange, ManifestChangeModel};
...
mod tests {
    ...
    use crate::product::model::ProductModel;
    ...
    fn recusa_quantidade_nao_positiva() {
        ...
        for bad in [0.0, -1.0, f64::NAN] {
            let error = ManifestTMImpl::new()
                ...
                .err()
                .expect("quantidade {bad} deveria ser recusada");
    ...
    fn so_desembarca_o_que_esta_carregando() {
        for status in [
            ...
            let error = ManifestTMImpl::new()
                ...
                .err()
                .expect("{status} não pode ser descarregado");
