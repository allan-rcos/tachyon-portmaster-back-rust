...
use crate::entity::manifest::{ManifestCargoEntity, ManifestCargoRow};
...
pub(crate) struct ManifestMariadbRepository;
...
impl ManifestRepository for ManifestMariadbRepository {
    async fn find_cargo(
        ...
        let raw_container = decode_id(container_id)?;
        let raw_product = decode_id(product_id)?;
