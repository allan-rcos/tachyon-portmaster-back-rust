...
use super::{decode_enum, decode_id, encode_id};
...
pub(crate) struct ContainerRow {
    ...
    /// Índice da variante de [`ContainerStatus`], nunca a string.
...
impl ContainerEntity {
    ...
    pub(crate) fn from_row(row: ContainerRow) -> anyhow::Result<Self> {
        Ok(Self {
            ...
            status: decode_enum(row.status, ContainerStatus::from_i32, "ContainerStatus")?,
