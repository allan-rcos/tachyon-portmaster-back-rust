...
//!
//! A exceção é [`opt_text`], onde a ausência é o dado: uma descrição de
...
pub(crate) fn id(row: &MySqlRow, column: &str) -> anyhow::Result<String> {
    Ok(encode_id(number(row, column)?))
}
