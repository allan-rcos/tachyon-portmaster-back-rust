...
//! ## Keyset e não offset
//!
//! Um `OFFSET` conta linhas a partir do começo, então uma inserção entre duas
//! páginas empurra tudo e faz a página seguinte repetir ou pular registros. O
//! token daqui carrega o **último id servido**, e a página seguinte é `id >
//! last_id` — o que foi inserido no meio-tempo não desloca nada.
//!
//! ## Por que o token guarda os filtros
//!
//! Um cursor só faz sentido dentro da consulta que o gerou. Se o cliente muda o
//! termo de busca mas reenvia o cursor antigo, continuar a varredura entregaria
//! uma página do conjunto velho sob o filtro novo. Comparando os filtros na
//! decodificação, um cursor incompatível é simplesmente **ignorado** e a
//! paginação recomeça — que é o comportamento que o chamador espera, e é por
//! isso que ele nunca precisa tratar cursor ruim como erro.
//!
//! ## O token é codificado, não assinado
//!
//! Base64url não é proteção: quem editar o id ganha outra página dos **mesmos**
//! dados que já podia ler. Não há nada a proteger aqui — a autorização acontece
//! no `app`, antes de a consulta existir. O que a codificação compra é um token
//! opaco, que atravessa uma querystring intacto e que ninguém do lado de fora
//! sente vontade de construir à mão.
...
///
/// `BTreeMap` e não `HashMap`: a serialização precisa ser estável para que o
/// mesmo conjunto de filtros produza sempre o mesmo JSON. Com ordem de iteração
/// aleatória, dois cursores idênticos poderiam não se reconhecer.
pub(crate) type CursorFilters = BTreeMap<String, String>;
...
/// inteiro, nunca lido de volta como número. Um `None` entra como string vazia
/// para que "sem filtro" seja um valor tão explícito quanto qualquer outro — se
/// a chave sumisse, dois filtros diferentes poderiam gerar o mesmo mapa.
pub(crate) fn filters<const N: usize>(pairs: [(&str, String); N]) -> CursorFilters {
    pairs
        .into_iter()
...
mod tests {
    ...
    fn busca(termo: &str) -> CursorFilters {
        filters([("limit", "20".to_owned()), ("search", termo.to_owned())])
    }
    ...
    fn a_ordem_dos_filtros_nao_muda_o_token() {
        ...
        let a = filters([("limit", "20".to_owned()), ("search", "cal".to_owned())]);
        let b = filters([("search", "cal".to_owned()), ("limit", "20".to_owned())]);
