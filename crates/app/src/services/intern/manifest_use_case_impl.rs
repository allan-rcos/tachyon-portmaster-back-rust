...
#[derive(Clone)]
pub(crate) struct ManifestUseCaseImpl<CR, PR, MR, T, C> {
...
impl<CR, PR, MR, T, C> ManifestUseCaseImpl<CR, PR, MR, T, C> {
    /// Monta o caso de uso.
...
impl<CR, PR, MR, T, C> ManifestUseCase for ManifestUseCaseImpl<CR, PR, MR, T, C>
where
    CR: ContainerRepository + Send + Sync,
    PR: ProductRepository + Send + Sync,
    MR: ManifestRepository + Send + Sync,
