...
//! [`portmaster_infra::database::scope`].

use std::future::Future;

use portmaster_infra::database::{scope, UnitOfWork};
...
pub(crate) async fn transaction<U, T, F>(unit_of_work: &U, body: F) -> Result<T, AppError>
where
    U: UnitOfWork,
...
mod tests {
    ...
    async fn o_sucesso_confirma_e_nao_desfaz() {
        ...
        let value = transaction(&uow, async { Ok::<_, AppError>(42) })
            .await
    ...
    async fn a_falha_desfaz_e_nao_confirma() {
        ...
        let error = transaction(&uow, async {
    ...
    async fn o_corpo_roda_dentro_do_escopo_de_transacao() {
        ...
        let dentro = transaction(&uow, async {
            Ok::<_, AppError>(portmaster_infra::database::in_scope())
        })
        .await
        .expect("o corpo não falhou");
