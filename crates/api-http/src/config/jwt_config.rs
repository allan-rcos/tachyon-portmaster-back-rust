...
/// eram seis variáveis a mais e viraram
/// [`SessionPolicy`](crate::ports::session_policy::SessionPolicy): não são
/// segredo nem identidade de deploy, são o que a API promete, e duas instâncias
/// da mesma versão não deveriam poder discordar sobre isso.
///
/// O padrão do `secret` é vazio de propósito — não existe padrão seguro para um
/// segredo de assinatura. Quem o exige é o
